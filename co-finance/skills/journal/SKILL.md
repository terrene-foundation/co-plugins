...
Manage the project journal — your analytical knowledge trail capturing decisions, discoveries, trade-offs, risks, connections, and gaps.

Parse `$ARGUMENTS`:
- **Empty or "status"**: Show journal status
- **"new TYPE topic"**: Create a new entry (e.g., `new DECISION chose-dcf-over-comparables`)
...
1. Determine the active workspace (or most recently modified under `workspaces/`, excluding `instructions/`)
...
4. Confirm with entry number and path
